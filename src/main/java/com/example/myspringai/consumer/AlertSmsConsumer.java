...
/**
 * 短信通知消费者（MQ 快路径，MANUAL ACK）。
 * <p>
 * Level 3（严重）→ 立即发送，成功 ACK，失败抛异常 → 重试 3 次 → DLQ<br>
 * Level 1/2（轻微/中等）→ 直接 ACK，DB 保持 PENDING，由 AlertSmsService 定时批量发送
 * <p>
 * 不再使用内存缓冲区攒批——进程崩溃零丢失。
 */
@Slf4j
    ...
    public void handleSms(AlarmEvent event,
        ...
        if (level >= 3) {
            ...
            try {
                ...
                log.info("[短信·紧急] 发送成功: alarmId={}", event.getAlarmId());
            } catch (Exception e) {
                log.error("[短信·紧急] 发送失败，等待重试: alarmId={}", event.getAlarmId(), e);
                throw e; // 抛异常 → smsRetryInterceptor 重试 3 次 → 耗尽进 DLQ
            }
        } else {
            // Level 1/2：直接 ACK，DB 里 sms_status=PENDING 由定时任务批量发送
            channel.basicAck(tag, false);
            log.debug("[短信·普通] 已ACK，等待定时任务批量发送: alarmId={}", event.getAlarmId());
        }
    }
