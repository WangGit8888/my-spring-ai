...
/**
 * 短信批量发送服务 — DB 驱动攒批，无内存缓冲区。
 * <p>
 * MQ 消费者 AlertSmsConsumer 只处理 Level 3 立即发送。<br>
 * Level 1/2 的批量发送全部由本服务负责：扫描 PENDING 超过 60s 的记录，合并发送。
 * <p>
 * 同时兜底 Level 3 超过 120s 仍 PENDING 的记录（MQ 消费者重试耗尽进 DLQ 的情况）。
 */
@Slf4j
@Component
...
public class AlertSmsService {
    ...
    private static final int FALLBACK_SECONDS = 120;
    ...
    public void processPendingSms() {
        ...
        if (!batchList.isEmpty()) {
            try {
                ...
                log.info("[短信·批量] 发送成功: {} 条", batchList.size());
            } catch (Exception e) {
        ...
        // 2. Level 3 兜底：超过 120s 仍 PENDING（MQ 消费者处理失败进 DLQ 了）
        ...
        for (AlertInfo alert : fallbackList) {
            try {
                ...
                alert.setSmsStatus("SUCCESS");
                alertInfoMapper.updateById(alert);
                log.warn("[短信·兜底] Level3超时未发，兜底发送成功: alarmId={}", alert.getAlertId());
            } catch (Exception e) {
    ...
    private void sendImmediately(AlertInfo alert) {
        // TODO: 对接短信服务发送短信
        log.info("[短信·紧急] 发送: alarmId={}, type={}, level={}, device={}",
